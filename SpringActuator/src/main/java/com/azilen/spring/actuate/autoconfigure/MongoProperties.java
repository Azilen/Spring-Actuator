...
public class MongoProperties {
	...
	public String getMongoClientDatabase() {
		return this.database != null ? this.database : (new MongoClientURI(
				this.uri)).getDatabase();
	}
	...
	public MongoClient createMongoClient(MongoClientOptions options,
		...
		try {
			if (!this.hasCustomAddress() && !this.hasCustomCredentials()) {
				MongoClient credentials1 = new MongoClient(new MongoClientURI(
						this.uri, this.builder(options)));
				return credentials1;
			}
			...
			if (this.hasCustomCredentials()) {
				host = this.authenticationDatabase == null ? this
			...
			arg5 = new MongoClient(Collections.singletonList(new ServerAddress(
					host, port)), credentials, options);
		} finally {
			this.clearPassword();
		}
	...
	private Builder builder(MongoClientOptions options) {
		return options != null ? MongoClientOptions.builder(options)
